...
const CategoryFilter = () => {
  const { categories, filterByCategories, filters } = useStories();
  ...
  const toggleAll = () => {
    // Se todos (ou mais) estiverem selecionados, desmarca todos.
  ...
  // Detect click outside to close and apply
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (menuRef.current && !menuRef.current.contains(event.target)) {
        if (isOpen) {
          // Quando clica fora, aplica e fecha (conforme pedido: "clica fora da janela e ja filtra automaticamente")
          filterByCategories(selected);
          setIsOpen(false);
        }
      }
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full left-0 w-full bg-slate-900/60 border border-white/10 border-t-0 rounded-b-3xl shadow-[0_32px_64px_-16px_rgba(0,0,0,0.8)] overflow-hidden backdrop-blur-3xl z-0"
          ...
          >
            ...
            <div className="max-h-[320px] overflow-y-auto p-2 space-y-1 custom-scrollbar scrollbar-thin scrollbar-thumb-white/10">
              {categories.map((cat) => {
